...
![Biểu đồ Train và Dev Loss](loss_curve.png)
...
![Biểu đồ Train và Dev Accuracy](accuracy_curve.png)
...
Bảng hiện tại ghi nhận cấu hình baseline đã được chạy và có đầy đủ số liệu. Các cấu hình khác cần được bổ sung nếu nhóm tiếp tục chạy thí nghiệm thay đổi learning rate, hidden size hoặc số iteration.

---

### 3.4.5. Lựa chọn cấu hình cuối cùng

Dựa trên kết quả hiện có, cấu hình được chọn cho báo cáo là mô hình Fully Connected với kiến trúc $784 \rightarrow 128 \rightarrow 10$, learning rate $\alpha = 0.1$ và quá trình huấn luyện đến iteration $490$. Cấu hình này đạt train accuracy $92.37\%$ và dev accuracy $92.00\%$, đồng thời có train loss và dev loss gần nhau, cho thấy mô hình đạt cân bằng tốt giữa khả năng học trên train và khả năng tổng quát hóa trên dev.

---

## 3.5. Đánh giá bằng confusion matrix

Confusion matrix giúp phân tích hiệu năng của mô hình theo từng lớp chữ số. Đây là công cụ quan trọng để phát hiện các nhóm chữ số thường bị nhầm lẫn với nhau, bổ sung góc nhìn chi tiết hơn so với accuracy tổng thể.
...
Confusion matrix được xây dựng trên tập dev có nhãn. Trong ma trận này, mỗi hàng biểu diễn nhãn thật và mỗi cột biểu diễn nhãn dự đoán. Các giá trị lớn trên đường chéo chính cho thấy mô hình dự đoán đúng phần lớn mẫu của lớp tương ứng; các giá trị ngoài đường chéo thể hiện những trường hợp mô hình nhầm lớp.

---

### 3.5.2. Các lớp chữ số dễ nhận diện

Các lớp có giá trị tập trung mạnh trên đường chéo chính là những lớp mô hình nhận diện tốt hơn. Thông thường, các chữ số có hình dạng đặc trưng rõ, ít phụ thuộc vào nét nối hoặc vùng khép kín sẽ dễ được mô hình phân biệt hơn. Việc quan sát confusion matrix giúp xác định trực tiếp lớp nào có tỷ lệ dự đoán đúng cao trên tập dev.

---

### 3.5.3. Các cặp chữ số dễ nhầm lẫn

Các giá trị ngoài đường chéo trong confusion matrix phản ánh các cặp chữ số dễ bị nhầm. Với bài toán chữ số viết tay, các cặp như $4$ và $9$, $1$ và $7$, hoặc $3$ và $5$ thường có nguy cơ nhầm lẫn do có cấu trúc nét viết tương đồng trong một số phong cách viết.

---

### 3.5.4. Nhận xét tổng quan từ confusion matrix

Confusion matrix cho thấy accuracy tổng thể cần được đọc cùng phân bố lỗi theo từng lớp. Một mô hình có accuracy cao vẫn có thể yếu ở một số cặp chữ số cụ thể. Do mô hình Fully Connected xử lý ảnh sau khi flatten, mô hình không trực tiếp khai thác quan hệ không gian cục bộ như các kiến trúc tích chập, vì vậy các chữ số có hình dạng gần nhau vẫn có thể gây nhầm lẫn.

---

## 3.6. Phân tích lỗi chi tiết

Phân tích lỗi chi tiết giúp chuyển các sai số định lượng thành nhận xét có ý nghĩa về dữ liệu và mô hình. Các mẫu dự đoán sai tiêu biểu được minh họa trong hình dưới đây.
...
Số $4$ và số $9$ có thể bị nhầm khi nét viết của số $4$ tạo thành vùng gần khép kín hoặc khi số $9$ bị viết hở. Khi ảnh được flatten thành vector, mô hình Fully Connected có thể học tương quan pixel tổng quát nhưng không có cơ chế chuyên biệt để nhận diện cấu trúc hình học cục bộ của vòng khép kín.

---

### 3.6.2. Trường hợp nhầm số 1 và số 7

Số $1$ và số $7$ thường bị nhầm khi số $1$ có nét nghiêng hoặc có gạch ngang phía trên. Trong các trường hợp này, phân bố pixel của hai chữ số có thể trở nên tương đồng, đặc biệt khi chữ số bị lệch tâm hoặc nét viết quá mảnh.

---

### 3.6.3. Trường hợp nhầm số 3 và số 5

Số $3$ và số $5$ có thể chia sẻ các vùng nét cong tương tự ở phần thân giữa và phần dưới. Nếu nét viết không rõ ràng hoặc phần đầu của số $5$ bị cong giống số $3$, mô hình có thể gán xác suất cao cho lớp sai.

---

### 3.6.4. Nguyên nhân sai số do nét viết và biến dạng hình học

Các lỗi dự đoán thường xuất hiện khi ảnh có nét viết nghiêng, dày hoặc mảnh bất thường, lệch tâm, thiếu nét hoặc có hình dạng không điển hình. Những biến dạng này làm mẫu đầu vào khác với phân phối phổ biến trong tập huấn luyện, khiến xác suất Softmax bị phân tán hoặc nghiêng về lớp có hình dạng gần hơn.

---

### 3.6.5. Nguyên nhân sai số do ảnh bị flatten

Khi ảnh hai chiều được flatten thành vector một chiều, quan hệ không gian cục bộ giữa các pixel không còn được biểu diễn trực tiếp. Đây là hạn chế quan trọng của ANN Fully Connected so với các mô hình khai thác cấu trúc ảnh tốt hơn. Vì vậy, mô hình có thể gặp khó khăn với các chữ số bị biến dạng hình học hoặc có nét viết gần giống nhau.

---

## 3.7. Trực quan hóa mô hình

Trực quan hóa mô hình giúp người đọc hiểu rõ hơn mô hình đã học được gì và cách nó đưa ra dự đoán. Trong phạm vi báo cáo hiện tại, các trực quan chính gồm biểu đồ loss, biểu đồ accuracy, confusion matrix và một số mẫu dự đoán sai tiêu biểu.

---

### 3.7.1. Trực quan hóa trọng số tầng đầu vào

Trọng số kết nối từ input tới hidden layer có thể được reshape về dạng ảnh $28 \times 28$ để quan sát mô hình phản ứng với các vùng pixel nào. Phần trực quan hóa trọng số chưa được cung cấp dưới dạng file ảnh trong dữ liệu đầu vào hiện tại, nên có thể bổ sung ở phụ lục nếu nhóm tạo thêm hình minh họa.

---

### 3.7.2. Trực quan hóa xác suất Softmax của một mẫu dự đoán

Biểu đồ xác suất Softmax cho thấy mức độ tự tin của mô hình đối với từng lớp chữ số. Trong báo cáo hiện tại, xác suất Softmax riêng cho từng mẫu chưa được cung cấp thành file ảnh, nhưng cơ chế dự đoán đã được thể hiện qua `np.argmax(A2, 0)` và các ảnh dự đoán sai tiêu biểu.

---

### 3.7.3. So sánh mẫu dự đoán đúng và sai

Các mẫu dự đoán sai được trình bày trong `misclassified_samples.png`. Việc so sánh các mẫu sai với phân bố lỗi trong confusion matrix giúp nhận diện những trường hợp mô hình dễ nhầm do nét viết bất thường hoặc do các chữ số có cấu trúc tương đồng.

---

### 3.7.4. Nhận xét khả năng học đặc trưng của mô hình

Kết quả thực nghiệm cho thấy mô hình Fully Connected đã học được các đặc trưng pixel tổng quát đủ để đạt dev accuracy $92.00\%$. Tuy nhiên, vì mô hình không khai thác trực tiếp cấu trúc không gian cục bộ, các trường hợp chữ số biến dạng hoặc có hình dạng gần nhau vẫn có thể gây lỗi dự đoán.

---

## 3.8. Triển khai dự đoán trên nét vẽ thực tế Web Demo

Phần Web Demo được định hướng để kiểm thử mô hình trên dữ liệu nét vẽ tự do của người dùng. Trọng tâm của phần này là quy trình thu nhận nét vẽ, tiền xử lý ảnh, chạy inference và trực quan hóa kết quả dự đoán. Trong dữ liệu đầu vào hiện tại chưa có ảnh pipeline Web Demo, vì vậy phần này mô tả quy trình kỹ thuật dự kiến.

---

### 3.8.1. Công cụ thu thập nét vẽ từ người dùng

Canvas được xem như một vùng thu nhận nét vẽ tự do, tạo ra ảnh đầu vào ban đầu để kiểm thử mô hình ngoài tập MNIST. Ảnh này là dữ liệu thô và cần được tiền xử lý trước khi đưa vào mạng.

---

### 3.8.2. Khác biệt giữa ảnh vẽ thực tế và ảnh MNIST

Ảnh vẽ thực tế có thể khác MNIST về màu nền, độ dày nét, vị trí chữ số, kích thước vùng vẽ và mức độ căn giữa. Sự khác biệt này tạo ra khoảng cách phân phối giữa dữ liệu huấn luyện và dữ liệu sử dụng thực tế.

---

### 3.8.3. Chuyển ảnh vẽ sang grayscale

Ảnh thu được từ Canvas có thể ở dạng nhiều kênh màu, trong khi mô hình được huấn luyện với ảnh xám một kênh. Vì vậy, ảnh cần được chuyển sang grayscale để đồng nhất định dạng đầu vào.

---

### 3.8.4. Nghịch đảo màu về dạng nền đen chữ trắng

Dữ liệu MNIST thường biểu diễn chữ số sáng trên nền tối. Nếu ảnh người dùng vẽ có nền sáng và nét tối, cần nghịch đảo màu để đưa ảnh về dạng tương đồng hơn với dữ liệu huấn luyện.

---

### 3.8.5. Tìm bounding box vùng chứa chữ số

Bounding box được dùng để xác định vùng pixel có nét vẽ, từ đó loại bỏ khoảng trống thừa xung quanh chữ số. Bước này giúp chữ số chiếm tỷ lệ hợp lý hơn trong khung ảnh trước khi resize.

---

### 3.8.6. Cắt ảnh và căn giữa chữ số

Sau khi xác định bounding box, chữ số được crop và căn giữa trong khung ảnh. Việc căn giữa giúp giảm sai lệch vị trí, một yếu tố có thể ảnh hưởng mạnh đến mô hình Fully Connected.

---

### 3.8.7. Resize ảnh về kích thước $28 \times 28$

Mô hình nhận đầu vào cố định gồm $784$ pixel, vì vậy ảnh sau khi xử lý cần được resize về kích thước $28 \times 28$. Đây là bước bắt buộc để ảnh tương thích với shape của `W1`.

---

### 3.8.8. Normalize và flatten thành vector 784 chiều

Sau khi resize, pixel được chia cho $255$ để đưa về khoảng $[0,1]$. Ảnh $28 \times 28$ sau đó được flatten thành vector $784$ chiều trước khi truyền vào hàm lan truyền tiến.

---

### 3.8.9. Chạy inference bằng trọng số đã huấn luyện

Ở giai đoạn inference, mô hình sử dụng `W1`, `b1`, `W2`, `b2` đã lưu để dự đoán mà không huấn luyện lại. Quá trình này chỉ thực hiện forward propagation và lấy nhãn bằng lớp có xác suất lớn nhất.

---

### 3.8.10. Trực quan hóa ảnh trước và sau tiền xử lý

Chuỗi ảnh trước và sau tiền xử lý chưa được cung cấp trong dữ liệu đầu vào hiện tại. Khi hoàn thiện Web Demo, phần này nên chèn các ảnh gồm ảnh vẽ gốc, ảnh grayscale, ảnh invert, ảnh crop/center và ảnh $28 \times 28$ cuối cùng.

---

### 3.8.11. Biểu đồ xác suất Softmax đầu ra

Biểu đồ xác suất Softmax cho dữ liệu vẽ tay chưa được cung cấp trong dữ liệu đầu vào hiện tại. Khi bổ sung, biểu đồ này nên thể hiện xác suất của cả 10 lớp để phân tích mức độ tự tin của mô hình.

---

### 3.8.12. Nhận xét kết quả trên nét vẽ thực tế

Kết quả trên nét vẽ thực tế phụ thuộc mạnh vào chất lượng tiền xử lý. Nếu ảnh được căn giữa, resize và normalize tốt, mô hình có nhiều khả năng dự đoán đúng hơn. Ngược lại, nét vẽ lệch tâm, quá dày, quá mảnh hoặc khác biệt nhiều so với MNIST có thể làm giảm độ chính xác.

---

## 3.9. Đánh giá tổng hợp thực nghiệm

Kết quả thực nghiệm cho thấy mô hình ANN Fully Connected tự cài đặt bằng `NumPy` có thể học hiệu quả trên dữ liệu MNIST. Với cấu hình baseline, mô hình đạt dev accuracy $92.00\%$ tại iteration $490$, đồng thời train loss và dev loss đều ở mức thấp và gần nhau.

---

### 3.9.1. Cấu hình mô hình tốt nhất

Cấu hình được chọn là mô hình Fully Connected với kiến trúc $784 \rightarrow 128 \rightarrow 10$, hidden size $128$, learning rate $\alpha = 0.1$ và quá trình huấn luyện đến iteration $490$. Kết quả đạt được là train accuracy $92.37\%$, dev accuracy $92.00\%$, train loss $0.2746$ và dev loss $0.2737$.

---

### 3.9.2. Ưu điểm của mô hình sau thực nghiệm

Mô hình có ưu điểm là cấu trúc đơn giản, dễ cài đặt, dễ phân tích và tốc độ inference nhanh. Với dev accuracy $92.00\%$, mô hình cho thấy khả năng học đặc trưng chữ số viết tay tương đối tốt dù chỉ sử dụng một tầng ẩn Fully Connected.

---

### 3.9.3. Hạn chế rút ra từ thực nghiệm

Hạn chế chính của mô hình là chưa khai thác trực tiếp quan hệ không gian cục bộ giữa các pixel. Do đó, mô hình vẫn có thể nhầm lẫn giữa các chữ số có hình dạng gần nhau hoặc các mẫu có nét viết biến dạng. Ngoài ra, khi áp dụng cho nét vẽ thực tế, chất lượng tiền xử lý ảnh có ảnh hưởng rất lớn đến kết quả dự đoán.

---

### 3.9.4. Bài học kỹ thuật từ quá trình thử nghiệm

Quá trình thực nghiệm cho thấy việc lựa chọn learning rate, kích thước tầng ẩn và số iteration ảnh hưởng trực tiếp đến chất lượng mô hình. Bên cạnh đó, các công cụ trực quan như biểu đồ loss, biểu đồ accuracy, confusion matrix và ảnh dự đoán sai giúp đánh giá mô hình đầy đủ hơn so với chỉ nhìn vào accuracy tổng thể.
