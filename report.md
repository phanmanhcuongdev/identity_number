...
Trong lần huấn luyện cuối cùng, mô hình ANN với kiến trúc $784 \rightarrow 128 \rightarrow 10$ đạt train accuracy $92.37\%$ và dev accuracy $92.00\%$ tại iteration $490$. Do đó, confusion matrix được sử dụng để làm rõ hơn chất lượng của mô hình trên từng chữ số và chỉ ra các dạng lỗi còn tồn tại sau khi mô hình đã hội tụ tương đối tốt.
...
Kết quả thực nghiệm cho thấy mô hình ANN Fully Connected đã học được các đặc trưng pixel đủ mạnh để đạt train accuracy $92.37\%$ và dev accuracy $92.00\%$ tại iteration $490$. Điều này chứng tỏ mô hình có khả năng phân biệt phần lớn chữ số MNIST dù chỉ sử dụng một tầng ẩn với $128$ neuron.
...
Tổng hợp các kết quả thực nghiệm cho thấy mô hình ANN Fully Connected tự cài đặt bằng `NumPy` đã hoàn thành tốt mục tiêu nhận diện chữ số viết tay MNIST. Với kiến trúc $784 \rightarrow 128 \rightarrow 10$, learning rate $\alpha = 0.1$ và $500$ iterations, mô hình đạt kết quả cuối tại iteration $490$ gồm train loss $0.2746$, dev loss $0.2737$, train accuracy $92.37\%$ và dev accuracy $92.00\%$.
...
Cấu hình được chọn là mô hình Fully Connected với kiến trúc $784 \rightarrow 128 \rightarrow 10$, hidden size $128$, learning rate $\alpha = 0.1$ và $500$ iterations. Kết quả cuối cùng được ghi nhận tại iteration $490$ là train accuracy $92.37\%$, dev accuracy $92.00\%$, train loss $0.2746$ và dev loss $0.2737$.
...
Đề tài đã hoàn thành mục tiêu cốt lõi: tự xây dựng mô hình ANN Fully Connected, huấn luyện bằng Batch Gradient Descent, đánh giá bằng loss/accuracy, phân tích confusion matrix, phân tích mẫu sai và mở rộng sang kiểm thử trên Wild Data. Kết quả cuối tại iteration $490$ đạt train loss $0.2746$, dev loss $0.2737$, train accuracy $92.37\%$ và dev accuracy $92.00\%$.
...
Kết quả thực nghiệm cho thấy mô hình hội tụ ổn định. Train loss và dev loss đều giảm về mức thấp, trong khi train accuracy và dev accuracy lần lượt đạt $92.37\%$ và $92.00\%$. Khoảng cách nhỏ giữa hai accuracy cho thấy mô hình không bị overfitting nghiêm trọng ở cấu hình cuối cùng.
...
Với cấu hình tốt nhất trong báo cáo, mô hình đạt train accuracy $92.37\%$ và dev accuracy $92.00\%$ tại iteration $490$. Train loss đạt $0.2746$, dev loss đạt $0.2737$. Sự tương đồng giữa train loss và dev loss cho thấy quá trình tối ưu ổn định và mô hình có khả năng tổng quát hóa tốt trên tập dev.
...
```python
[Chèn toàn bộ code ann.py vào đây]
```
...
Phụ lục này tổng hợp kết quả theo các cấu hình learning rate. Cấu hình $\alpha = 0.1$ là cấu hình tốt nhất hiện tại trong báo cáo. Các cấu hình $\alpha = 0.01$ và $\alpha = 0.5$ được giữ lại để điền sau khi chạy thêm thực nghiệm.

| Learning rate $\alpha$ | Hidden size | Iterations | Train loss | Dev loss | Train accuracy | Dev accuracy | Ghi chú |
|---:|---:|---:|---:|---:|---:|---:|---|
| 0.01 | 128 | 500 | [Điền số] | [Điền số] | [Điền số] | [Điền số] | Cần chạy bổ sung để đánh giá tốc độ hội tụ. |
| 0.1 | 128 | 500 | 0.2746 | 0.2737 | 92.37% | 92.00% | Cấu hình tốt nhất hiện tại, hội tụ ổn định. |
| 0.5 | 128 | 500 | [Điền số] | [Điền số] | [Điền số] | [Điền số] | Cần chạy bổ sung để kiểm tra dao động hoặc mất ổn định. |
