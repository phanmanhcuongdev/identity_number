...
Dữ liệu huấn luyện được lưu tại `data/train_infinity.npy` và nhãn tại `data/labels_infinity.npy`. Model cuối cùng được lưu tại `weights/model_infinity.npz`. `ann.py` chỉ chứa logic inference, còn `scripts/train_infinity.py` chứa logic huấn luyện và backpropagation.

---

## 1.3. Định hướng giải pháp

Định hướng giải pháp của đề tài gồm năm bước chính: chuẩn bị dữ liệu, xây dựng mô hình, huấn luyện, đánh giá và kiểm thử trực quan. Dữ liệu Infinity được lưu dưới dạng `.npy`, gồm `data/train_infinity.npy` và `data/labels_infinity.npy`, sau đó được chuẩn hóa pixel về khoảng $[0,1]$. Mô hình ANN được khởi tạo với ba tầng ẩn và được huấn luyện bằng mini-batch Adam Optimizer.
...
### 1.3.1. Huấn luyện ANN $784 \rightarrow 1024 \rightarrow 512 \rightarrow 256 \rightarrow 10$ trên MNIST
...
Dữ liệu được đọc từ hai file `data/train_infinity.npy` và `data/labels_infinity.npy`. Trong `scripts/train_infinity.py`, dữ liệu đặc trưng được nạp bằng `np.load`, nhãn được reshape về vector một chiều và pixel được chuẩn hóa về khoảng $[0,1]$ nếu dữ liệu ban đầu còn ở thang $0$ đến $255$.
...
Quá trình huấn luyện trong `scripts/train_infinity.py` sử dụng Adam Optimizer. Adam duy trì moment bậc nhất và moment bậc hai cho từng tham số, giúp learning rate hiệu dụng thích nghi theo từng trọng số.
...
Dữ liệu train được chia thành mini-batch kích thước $4096$. Cách này giúp mô hình học ổn định và phù hợp hơn với tập dữ liệu 10 triệu mẫu.

---

### 2.5.4. Cập nhật tham số bằng Adam Optimizer

Adam cập nhật tham số bằng moment bậc nhất, moment bậc hai và bias correction. Learning rate tại epoch $e$ là:
...
Thiết kế thực nghiệm gồm: nạp dữ liệu `.npy`, chuẩn hóa pixel, chia train/validation, khởi tạo tham số, huấn luyện bằng Adam Optimizer, đánh giá theo từng epoch và sinh ảnh báo cáo bằng `scripts/generate_report_assets.py`.
...
Dữ liệu được đọc từ `data/train_infinity.npy` và `data/labels_infinity.npy`. Với 10,000,000 mẫu, cấu hình huấn luyện dùng 9,900,000 mẫu train và 100,000 mẫu validation.
...
Trọng số được khởi tạo bằng He Initialization để phù hợp với các tầng ReLU.
...
Biểu đồ cho thấy training loss giảm đều theo epoch, còn validation loss ổn định quanh mức $0.039$ ở giai đoạn cuối.
...
Biểu đồ accuracy cho thấy training accuracy đạt $99.44\%$ và validation accuracy đạt $98.82\%$ ở epoch 20.
...
Các giá trị lớn trên đường chéo chính thể hiện mô hình nhận diện đúng phần lớn mẫu, phù hợp với validation accuracy $98.82\%$ ở epoch 20.
...
Tổng hợp kết quả thực nghiệm cho thấy mô hình Deep ANN Fully Connected tự cài đặt bằng `NumPy` đã đạt hiệu năng cao trên Infinity Dataset. Với kiến trúc $784 \rightarrow 1024 \rightarrow 512 \rightarrow 256 \rightarrow 10$, Adam Optimizer, 20 epoch và batch size $4096$, mô hình đạt training accuracy $99.44\%$, validation accuracy $98.82\%$ và validation loss $0.0393$.

---

### 3.9.1. Cấu hình mô hình tốt nhất

Cấu hình được chọn là Deep ANN $784 \rightarrow 1024 \rightarrow 512 \rightarrow 256 \rightarrow 10$, huấn luyện bằng Adam Optimizer với learning rate ban đầu $0.001$ và decay $0.92$ mỗi epoch. Model cuối cùng được lưu tại `weights/model_infinity.npz`.
...
Đề tài đã xây dựng thành công mô hình Deep ANN Fully Connected bằng `NumPy` với kiến trúc $784 \rightarrow 1024 \rightarrow 512 \rightarrow 256 \rightarrow 10$. Mô hình được huấn luyện bằng Adam Optimizer trên Infinity Dataset và đạt training accuracy $99.44\%$, validation accuracy $98.82\%$, validation loss $0.0393$ ở epoch 20.
...
Phụ lục trình bày các nội dung kỹ thuật bổ trợ cho phần chính của báo cáo, bao gồm mã nguồn inference, mô tả script huấn luyện, bảng kết quả thực nghiệm, confusion matrix, mẫu dự đoán sai và pipeline tiền xử lý ảnh Canvas.
