...
Trong phiên bản cuối, `train.csv` là bộ dữ liệu mở rộng gồm $906{,}754$ mẫu huấn luyện có nhãn. File vẫn giữ định dạng Kaggle ban đầu với cột `label` và $784$ cột pixel, nên pipeline đọc dữ liệu trong `ann.py` không cần thay đổi cấu trúc.
...
Cấu hình huấn luyện cuối cùng trong `ann.py` được xác định tại đoạn `if __name__ == "__main__"`. Khi chạy trực tiếp file, chương trình chủ động xoá `model_weights.npz` cũ nếu tồn tại, sau đó huấn luyện lại mô hình từ đầu trên `train.csv`. Hàm `gradient_descent(X_train, Y_train, iterations=1200)` sử dụng Step Decay learning rate thay vì một giá trị $\alpha$ cố định.
...
Mô hình cuối cùng không dùng một learning rate cố định. Thay vào đó, `ann.py` gọi `get_learning_rate(i)` trong từng iteration để lấy $\alpha_i$ theo Step Decay. Giai đoạn đầu dùng $\alpha = 0.5$ để học nhanh, giai đoạn giữa giảm xuống $\alpha = 0.2$ để hội tụ ổn định hơn, và giai đoạn cuối dùng $\alpha = 0.05$ để tinh chỉnh tham số.

---

### 3.2.3. Số iteration mặc định

Số iteration của cấu hình huấn luyện cuối cùng là:

$$
1200
$$

Giá trị này được truyền trực tiếp trong:

```python
gradient_descent(X_train, Y_train, iterations=1200)
```
...
Các tham số được khởi tạo trong `init_params()` bằng He Initialization, phù hợp với tầng ẩn sử dụng ReLU. Cụ thể:
...
Điều này giúp phương sai của tín hiệu được kiểm soát tốt hơn khi đi qua tầng ReLU. Với tầng đầu tiên, hệ số tỷ lệ là $\sqrt{2/784}$; với tầng thứ hai, hệ số tỷ lệ là $\sqrt{2/128}$. Bias được khởi tạo bằng vector $0$ để giữ shape đúng nhưng không tạo lệch ban đầu.
...
Trong phiên bản huấn luyện cuối cùng, mô hình sử dụng hidden size $128$, số iteration $1200$ và Step Decay learning rate gồm các giai đoạn $0.5 \rightarrow 0.2 \rightarrow 0.05$. Các kết quả trước đó với learning rate cố định vẫn được giữ lại như lịch sử so sánh siêu tham số, nhưng cấu hình đang phản ánh đúng code cuối cùng là lịch học thích nghi theo iteration.

---

### 3.4.1. Ảnh hưởng của learning rate

Learning rate được điều chỉnh theo Step Decay. Với $i < 400$, mô hình dùng $\alpha = 0.5$ để tăng tốc học ở giai đoạn đầu. Với $400 \le i < 800$, learning rate giảm còn $\alpha = 0.2$ để giảm dao động khi mô hình đã học được đặc trưng chính. Từ $i \ge 800$, learning rate tiếp tục giảm còn $\alpha = 0.05$ nhằm tinh chỉnh tham số ở giai đoạn cuối.
...
Code hiện tại chạy cấu hình huấn luyện cuối cùng với số iteration mặc định là $1200$. Việc tăng số iteration kết hợp với Step Decay cho phép mô hình có giai đoạn học nhanh ban đầu, giai đoạn hội tụ trung gian và giai đoạn tinh chỉnh cuối cùng. Cách chia này phù hợp hơn với tập `train.csv` mở rộng có quy mô rất lớn, vì mô hình cần nhiều vòng cập nhật hơn để khai thác hết dữ liệu.
...
Dựa trên quá trình so sánh siêu tham số, learning rate $\alpha = 0.5$ cho thấy khả năng bứt phá tốt ở giai đoạn đầu. Vì vậy, cấu hình cuối cùng giữ $\alpha = 0.5$ cho $400$ iteration đầu, sau đó giảm theo lịch $0.2$ và $0.05$ để cải thiện độ ổn định. Cấu hình cuối cùng của code là kiến trúc $784 \rightarrow 128 \rightarrow 10$, hidden size $128$, Step Decay learning rate $0.5 \rightarrow 0.2 \rightarrow 0.05$ và $1200$ iterations.
...
Tổng hợp các kết quả thực nghiệm cho thấy mô hình ANN Fully Connected tự cài đặt bằng `NumPy` đã hoàn thành tốt mục tiêu nhận diện chữ số viết tay MNIST. Với kiến trúc $784 \rightarrow 128 \rightarrow 10$, hidden size $128$ và dữ liệu `train.csv` mở rộng, cấu hình cuối cùng trong code sử dụng $1200$ iterations cùng Step Decay learning rate $0.5 \rightarrow 0.2 \rightarrow 0.05$. Các kết quả trước đó tại iteration $490$ cho thấy $\alpha = 0.5$ là giai đoạn học nhanh hiệu quả, nên được chọn làm learning rate khởi đầu trong lịch huấn luyện cuối.
...
Cấu hình được chọn là mô hình Fully Connected với kiến trúc $784 \rightarrow 128 \rightarrow 10$, hidden size $128$, Step Decay learning rate $0.5 \rightarrow 0.2 \rightarrow 0.05$ và $1200$ iterations. Khi chạy trực tiếp `ann.py`, chương trình xoá `model_weights.npz` cũ để bảo đảm mô hình được huấn luyện mới hoàn toàn trên `train.csv`.
...
Quá trình thử nghiệm cho thấy chất lượng mô hình phụ thuộc đồng thời vào kiến trúc, siêu tham số, dữ liệu và tiền xử lý. Learning rate lớn như $\alpha = 0.5$ giúp mô hình học nhanh ở giai đoạn đầu, nhưng lịch giảm dần về $\alpha = 0.2$ và $\alpha = 0.05$ cần thiết để ổn định quá trình hội tụ ở các giai đoạn sau. Confusion matrix và các mẫu dự đoán sai vẫn là công cụ cần thiết để hiểu rõ mô hình còn yếu ở đâu.
...
Đề tài đã hoàn thành mục tiêu cốt lõi: tự xây dựng mô hình ANN Fully Connected, huấn luyện bằng Batch Gradient Descent, đánh giá bằng loss/accuracy, phân tích confusion matrix, phân tích mẫu sai và mở rộng sang kiểm thử trên Wild Data. Phiên bản code cuối cùng sử dụng `train.csv` mở rộng, tự xoá `model_weights.npz` cũ trước khi huấn luyện và chạy $1200$ iterations với Step Decay learning rate.
...
Kết quả thực nghiệm trước đó cho thấy mô hình hội tụ ổn định với $\alpha = 0.5$ ở giai đoạn đầu. Vì vậy, cấu hình cuối cùng tiếp tục dùng $\alpha = 0.5$ để bứt phá, sau đó giảm xuống $\alpha = 0.2$ và $\alpha = 0.05$ để tinh chỉnh. Thiết kế này giúp cân bằng giữa tốc độ học và độ ổn định khi huấn luyện trên tập dữ liệu lớn.
...
Với cấu hình cuối cùng trong code, mô hình được huấn luyện lại từ đầu trên `train.csv` bằng $1200$ iterations và Step Decay learning rate. Cách tổ chức này kế thừa kết quả thực nghiệm tốt của $\alpha = 0.5$ ở giai đoạn đầu, đồng thời bổ sung các giai đoạn giảm learning rate để tăng khả năng hội tụ ổn định trên tập dữ liệu mở rộng.
