...
Đề tài hướng đến việc xây dựng một mô hình ANN Fully Connected có khả năng phân loại ảnh chữ số viết tay trên Infinity Dataset gồm 10,000,000 mẫu, được lưu dưới định dạng nhị phân `.npy`. Phiên bản cuối cùng sử dụng kiến trúc Deep ANN gồm ba tầng ẩn và được đánh giá bằng training loss, validation loss, training accuracy, validation accuracy, confusion matrix và các mẫu dự đoán sai.

Phạm vi đề tài tập trung vào mô hình Fully Connected tự triển khai bằng `NumPy`, không sử dụng CNN, RNN, Transformer hoặc framework học sâu bậc cao cho phần lõi. Mô hình cuối cùng dùng kiến trúc $784 \rightarrow 1024 \rightarrow 512 \rightarrow 256 \rightarrow 10$ và được tối ưu bằng Adam Optimizer.
...
Đề tài đánh giá quá trình học bằng training loss, validation loss, training accuracy và validation accuracy theo từng epoch. Kết quả cuối cùng ở epoch 20 đạt training accuracy $99.44\%$, validation accuracy $98.82\%$ và validation loss $0.0393$.
...
Sau khi huấn luyện, mô hình được đánh giá bằng loss, accuracy, confusion matrix và các mẫu dự đoán sai. Ở bước mở rộng, mô hình được đưa vào Flask Web Demo để nhận diện chữ số do người dùng tự vẽ sau khi ảnh đã được tiền xử lý về định dạng gần với MNIST.
...
Training loss và training accuracy phản ánh mức độ mô hình học được trên tập huấn luyện, trong khi validation loss và validation accuracy phản ánh khả năng tổng quát hóa. Các chỉ số này được ghi theo từng epoch trong `train.log`.

---

### 1.3.3. Phân tích lỗi dự đoán bằng confusion matrix và mẫu sai

Confusion matrix và ảnh dự đoán sai được sinh bằng `scripts/generate_report_assets.py` trên 10,000 mẫu đầu tiên từ Infinity Dataset, sử dụng trọng số cuối cùng tại `weights/model_infinity.npz`.

---

### 1.3.4. Kiểm thử inference trên nét vẽ người dùng

Ảnh người dùng vẽ được tiền xử lý bằng `OpenCV`, resize về $28 \times 28$, normalize và flatten thành vector $784 \times 1$ trước khi đưa vào `forward_prop`.
...
Chương 2 trình bày cơ sở lý thuyết cần thiết để hiểu mô hình ANN Fully Connected dùng trong đề tài. Nội dung bao gồm biểu diễn dữ liệu MNIST, flatten ảnh $28 \times 28$ thành vector $784$ chiều, chuẩn hóa pixel, nhãn số nguyên từ $0$ đến $9$, kiến trúc mạng $784 \rightarrow 1024 \rightarrow 512 \rightarrow 256 \rightarrow 10$, lan truyền tiến, ReLU, Softmax, Cross-Entropy Loss, lan truyền ngược, Adam Optimizer và các chỉ số đánh giá như loss và accuracy.

---

### 1.4.2. Nội dung Chương 3

Chương 3 trình bày phần thực nghiệm và đánh giá kết quả. Nội dung chương bao gồm môi trường chạy, cách chia train/validation, cấu hình mô hình, kết quả huấn luyện theo epoch, phân tích hội tụ, confusion matrix, phân tích lỗi và Web Demo.

---

### 1.4.3. Nội dung Chương 4

Chương 4 tổng kết những kết quả đã đạt được của đề tài, bao gồm việc tự xây dựng mô hình ANN bằng `NumPy`, huấn luyện và đánh giá mô hình trên Infinity Dataset, cũng như định hướng kiểm thử trên dữ liệu vẽ tay thực tế. Chương này đồng thời chỉ ra các hạn chế của mô hình Fully Connected, hạn chế của Adam Optimizer và các rủi ro trong tiền xử lý ảnh vẽ tay. Từ đó, báo cáo đề xuất các hướng phát triển phù hợp như cải thiện pipeline tiền xử lý, bổ sung dữ liệu kiểm thử thực tế và nâng cao khả năng trực quan hóa kết quả.
...
Trong đó $\theta$ gồm bốn tầng trọng số và bias của mô hình Deep ANN.

---

### 2.1.1. Đầu vào của bài toán

Đầu vào của mô hình là ảnh grayscale kích thước $28 \times 28$, được flatten thành vector $784$ chiều. Vector này tương thích với ma trận trọng số đầu tiên `W1` có shape $(1024, 784)$.

---

### 2.1.2. Đầu ra của bài toán

Đầu ra là vector xác suất trên 10 lớp chữ số. Tầng cuối sử dụng `W4` shape $(10, 256)$ và `b4` shape $(10, 1)`. Hàm `get_predictions(A4)` chọn lớp có xác suất lớn nhất bằng `np.argmax(A4, 0)`.

---

### 2.1.3. Đặc điểm của phân loại đa lớp

Mỗi mẫu chỉ thuộc một lớp đúng trong 10 lớp chữ số. Trong huấn luyện, `scripts/train_infinity.py` tính Cross-Entropy Loss trực tiếp từ xác suất Softmax của lớp đúng.

---

## 2.2. Dữ liệu MNIST mở rộng và tiền xử lý

Dữ liệu được tạo bằng `scripts/generate_infinity.py` và được đọc từ hai file `data/train_infinity.npy` và `data/labels_infinity.npy`. Trong `scripts/train_infinity.py`, dữ liệu đặc trưng được nạp bằng `np.load`, nhãn được reshape về vector một chiều và pixel được chuẩn hóa về khoảng $[0,1]$ nếu dữ liệu ban đầu còn ở thang $0$ đến $255$.
...
Mạng Fully Connected nhận vector đặc trưng thay vì ma trận ảnh hai chiều. Do đó ảnh $28 \times 28$ được biểu diễn thành vector $784$ chiều trước khi đi vào tầng đầu vào.
...
### 2.2.4. Nhãn và Cross-Entropy Loss

Nhãn trong `data/labels_infinity.npy` là số nguyên từ $0$ đến $9$. Khi huấn luyện, `scripts/train_infinity.py` tính Cross-Entropy Loss trực tiếp từ xác suất Softmax tại lớp đúng của từng mẫu.

---

## 2.3. Mô hình ANN Fully Connected

Mô hình cuối cùng là Deep ANN Fully Connected gồm ba tầng ẩn và một tầng đầu ra. Phần inference nằm trong `ann.py`; phần huấn luyện, backpropagation và Adam update nằm trong `scripts/train_infinity.py`.
...
Các tham số được học gồm $W^{[1]}$, $b^{[1]}$, $W^{[2]}$, $b^{[2]}$, $W^{[3]}$, $b^{[3]}$, $W^{[4]}$ và $b^{[4]}$.
...
| $W^{[1]}$ | `W1` | $1024 \times 784$ | Trọng số từ input sang hidden 1 |
| $b^{[1]}$ | `b1` | $1024 \times 1$ | Bias hidden 1 |
| $W^{[2]}$ | `W2` | $512 \times 1024$ | Trọng số hidden 1 sang hidden 2 |
| $b^{[2]}$ | `b2` | $512 \times 1$ | Bias hidden 2 |
| $W^{[3]}$ | `W3` | $256 \times 512$ | Trọng số hidden 2 sang hidden 3 |
| $b^{[3]}$ | `b3` | $256 \times 1$ | Bias hidden 3 |
| $W^{[4]}$ | `W4` | $10 \times 256$ | Trọng số hidden 3 sang output |
| $b^{[4]}$ | `b4` | $10 \times 1$ | Bias output |
...
Backpropagation được triển khai trong `scripts/train_infinity.py` để tính gradient cho `W1`, `b1`, `W2`, `b2`, `W3`, `b3`, `W4` và `b4`.

---

### 2.5.2. Cross-Entropy Loss

Loss được tính bằng trung bình âm log xác suất của lớp đúng trong batch. Xác suất được lấy từ Softmax đầu ra.
...
### 2.5.4. Cập nhật tham số bằng Adam Optimizer

Adam cập nhật tham số bằng moment bậc nhất, moment bậc hai và bias correction. Với gradient $g_t$ tại bước cập nhật $t$, các đại lượng chính được tính như sau:
...
Trong đó $\hat{m}_t$ và $\hat{v}_t$ là các moment đã hiệu chỉnh bias, $\eta$ là learning rate tại thời điểm cập nhật. Learning rate tại epoch $e$ là:
...
Mô hình được đánh giá bằng training loss, training accuracy, validation loss và validation accuracy sau mỗi epoch. Ngoài ra, `scripts/generate_report_assets.py` sinh confusion matrix và các mẫu dự đoán sai để phục vụ phân tích trực quan.

---

### 2.6.1. Train accuracy và validation accuracy

Accuracy được tính bằng tỷ lệ dự đoán đúng trên tổng số mẫu. Ở epoch 20, mô hình đạt training accuracy $99.44\%$ và validation accuracy $98.82\%$.

---

### 2.6.2. Train loss và validation loss

Validation loss cuối cùng là $0.0393$. Giá trị loss thấp cùng với validation accuracy cao cho thấy mô hình hội tụ tốt trên tập validation.

---

### 2.6.3. Confusion matrix

Confusion matrix được sinh bằng `scripts/generate_report_assets.py` trên 10,000 mẫu đầu tiên từ `data/train_infinity.npy` và `data/labels_infinity.npy`, sử dụng trọng số cuối cùng `weights/model_infinity.npz`.
...
Các mẫu dự đoán sai được lấy từ cùng tập 10,000 mẫu đánh giá của Infinity Dataset. Hình minh họa giúp xác định các chữ số có hình dạng dễ gây nhầm lẫn cho mô hình.
...
Mã nguồn sử dụng `NumPy` cho tính toán ma trận, `Matplotlib` và `Seaborn` để trực quan hóa, `scikit-learn` để tính confusion matrix, cùng `Flask`, `Pillow` và `OpenCV` cho Web Demo.

---

### 3.1.2. Cách chia tập train/validation

Dữ liệu được tạo trước bằng `scripts/generate_infinity.py`, sau đó được đọc từ `data/train_infinity.npy` và `data/labels_infinity.npy`. Với 10,000,000 mẫu, cấu hình huấn luyện dùng 9,900,000 mẫu train và 100,000 mẫu validation.

---

### 3.1.3. Quy trình huấn luyện

Mỗi epoch, script shuffle tập train, chia mini-batch kích thước $4096$, thực hiện forward propagation, backpropagation và cập nhật tham số bằng Adam.

---

### 3.1.4. Quy trình đánh giá

Sau mỗi epoch, script đánh giá validation loss và validation accuracy. Các biểu đồ được sinh từ `train.log`.

---

## 3.2. Cấu hình mô hình cơ sở

Cấu hình cuối cùng là Deep ANN Fully Connected $784 \rightarrow 1024 \rightarrow 512 \rightarrow 256 \rightarrow 10$ huấn luyện bằng Adam Optimizer.

---

### 3.2.1. Kiến trúc $784 \rightarrow 1024 \rightarrow 512 \rightarrow 256 \rightarrow 10$

Shape trọng số trong `weights/model_infinity.npz`:
...
Learning rate ban đầu là $0.001$ và được nhân với decay $0.92$ sau mỗi epoch:
...
Cấu hình cuối cùng huấn luyện trong $20$ epoch với batch size $4096$ và $2417$ batch mỗi epoch.

---

### 3.2.4. Phương pháp khởi tạo tham số

Trọng số được khởi tạo bằng He Initialization, với độ lệch chuẩn tỷ lệ theo $\sqrt{2 / n_{in}}$, trong đó $n_{in}$ là số đầu vào của tầng hiện tại. Cách khởi tạo này phù hợp với ReLU vì giúp duy trì phương sai tín hiệu qua nhiều tầng, qua đó giảm nguy cơ activation bị suy giảm hoặc bùng nổ trong mạng sâu.
...
### 3.3.1. Bảng loss và accuracy theo epoch
...
### 3.3.2. Biểu đồ train/validation loss

![Biểu đồ Train/Dev Loss](loss_curve.png)

Biểu đồ cho thấy training loss giảm đều từ $0.1559$ xuống $0.0177$, trong khi validation loss giảm mạnh ở các epoch đầu và đạt $0.0393$ tại epoch 20. Khoảng cách giữa train loss và validation loss tồn tại nhưng không tăng đột biến, cho thấy mô hình chưa xuất hiện dấu hiệu overfitting nghiêm trọng. Quy mô 10,000,000 mẫu của Infinity Dataset góp phần giảm rủi ro overfitting so với các tập dữ liệu nhỏ hơn.

---

### 3.3.3. Biểu đồ train/validation accuracy

![Biểu đồ Train/Dev Accuracy](accuracy_curve.png)

Biểu đồ accuracy cho thấy training accuracy đạt $99.44\%$ và validation accuracy đạt $98.82\%$ ở epoch 20. Chênh lệch $0.62$ điểm phần trăm giữa hai chỉ số này là nhỏ so với quy mô mô hình, qua đó cho thấy khả năng tổng quát hóa tốt trên tập validation.

---

### 3.3.4. Nhận xét về tốc độ hội tụ

Mô hình đạt validation accuracy $97.20\%$ sau epoch 1 và vượt $98\%$ từ epoch 3. Từ nửa sau quá trình huấn luyện, validation accuracy ổn định quanh $98.7\%$ đến $98.8\%$.
...
| Starting LR | 0.001 |
| LR decay | 0.92 mỗi epoch |
| Train accuracy cuối | 99.44% |
| Validation accuracy cuối | 98.82% |
| Validation loss cuối | 0.0393 |

---

## 3.5. Đánh giá bằng confusion matrix

Confusion matrix được sinh bằng `scripts/generate_report_assets.py` trên 10,000 mẫu đầu tiên từ `data/train_infinity.npy` và `data/labels_infinity.npy`, sử dụng model cuối cùng tại `weights/model_infinity.npz`.

---

### 3.5.1. Xây dựng confusion matrix trên 10,000 mẫu đánh giá

![Confusion Matrix](confusion_matrix.png)

Các giá trị lớn trên đường chéo chính thể hiện mô hình nhận diện đúng phần lớn mẫu trong tập đánh giá 10,000 ảnh. Vì confusion matrix chỉ được sinh từ một tập con của Infinity Dataset, hình này đóng vai trò công cụ chẩn đoán trực quan, bổ sung cho validation accuracy tổng thể $98.82\%$ ở epoch 20.
...
Confusion matrix cho thấy mô hình đạt chất lượng cao nhưng vẫn còn lỗi ở một số mẫu khó. Đây là hạn chế tự nhiên của Fully Connected ANN khi ảnh bị flatten thành vector $784$ chiều.
...
Kết quả thực nghiệm cho thấy mô hình ANN Fully Connected đã học được các đặc trưng pixel đủ mạnh để đạt train accuracy $99.44\%$ và validation accuracy $98.82\%$ tại epoch $20$ trong cấu hình tốt nhất. Điều này chứng tỏ mô hình có khả năng phân biệt phần lớn chữ số trong Infinity Dataset, dù kiến trúc vẫn dựa trên biểu diễn Fully Connected của ảnh đã flatten.
...
Mô hình ANN yêu cầu đầu vào có đúng $784$ phần tử, tương ứng với ảnh $28 \times 28$. Vì vậy, ảnh sau khi crop và pad được resize về kích thước $28 \times 28$. Bước này bảo đảm shape của ảnh tương thích với ma trận trọng số $W^{[1]}$ có kích thước $1024 \times 784$.
...
Với dữ liệu Out-of-Distribution (OOD), biểu đồ Softmax đặc biệt hữu ích vì nó cho biết mô hình chỉ sai nhãn hay còn thiếu tự tin về cấu trúc đầu vào. Đây là thông tin quan trọng để cải thiện preprocessing và thu thập thêm dữ liệu kiểm thử.
...
Tổng hợp kết quả thực nghiệm cho thấy mô hình Deep ANN Fully Connected tự cài đặt bằng `NumPy` đạt hiệu năng cao trên Infinity Dataset. Các thông số chi tiết về kiến trúc, siêu tham số và kết quả cuối cùng đã được trình bày trong các mục 3.2 đến 3.4; phần này tập trung vào ý nghĩa thực nghiệm và các hạn chế chính của mô hình.

---

### 3.9.1. Cấu hình mô hình tốt nhất

Cấu hình được chọn là mô hình Infinity đã trình bày ở Chương 3, gồm ba tầng ẩn và được huấn luyện bằng Adam Optimizer trên dữ liệu `.npy`. Model cuối cùng được lưu tại `weights/model_infinity.npz`.

---

### 3.9.2. Ưu điểm của mô hình sau thực nghiệm

Mô hình đạt accuracy cao trong khi vẫn giữ được cách triển khai minh bạch bằng `NumPy`. Tốc độ inference nhanh vì dự đoán chỉ cần các phép nhân ma trận và hàm kích hoạt cơ bản.

---

### 3.9.3. Hạn chế rút ra từ thực nghiệm

Mô hình Fully Connected làm mất cấu trúc không gian hai chiều khi flatten ảnh. Vì vậy, mô hình vẫn có thể nhầm các chữ số có cấu trúc gần nhau.

---

### 3.9.4. Bài học kỹ thuật từ quá trình thử nghiệm

Hiệu quả của mô hình phụ thuộc đồng thời vào kiến trúc, dữ liệu, optimizer và preprocessing. Với dữ liệu thực tế từ Canvas, preprocessing là thành phần bắt buộc để giảm khác biệt phân phối giữa dữ liệu người dùng và dữ liệu huấn luyện.
...
Đề tài đã xây dựng thành công mô hình Deep ANN Fully Connected bằng `NumPy` và huấn luyện mô hình Infinity bằng Adam Optimizer trên dữ liệu `.npy`. Các thông số kiến trúc, siêu tham số và kết quả định lượng cuối cùng đã được trình bày chi tiết trong Chương 3.

---

### 4.1.1. Kết quả xây dựng ANN NumPy

`ann.py` được module hóa cho inference, bao gồm ReLU, Softmax, `forward_prop`, `load_model` và `get_predictions`. Logic huấn luyện và cập nhật tham số nằm trong `scripts/train_infinity.py`.

---

### 4.1.2. Kết quả đánh giá thực nghiệm

Confusion matrix và các mẫu dự đoán sai cho thấy mô hình nhận diện đúng phần lớn mẫu trong tập đánh giá 10,000 ảnh từ Infinity Dataset. Các lỗi còn lại chủ yếu xuất hiện ở những chữ số có hình dạng gần nhau.

---

### 4.1.3. Kết quả kiểm thử trên nét vẽ thực tế

Web Demo sử dụng Flask để nhận ảnh Canvas, tiền xử lý bằng `OpenCV`, sau đó gọi model tại `weights/model_infinity.npz` để dự đoán.

---

### 4.1.4. Đóng góp nổi bật của đề tài

Đề tài đóng góp một pipeline hoàn chỉnh gồm huấn luyện Deep ANN, lưu model, sinh ảnh báo cáo tự động và triển khai Web Demo nhận diện chữ số viết tay.

---

## 4.2. Hạn chế

Dù đạt kết quả cao, mô hình vẫn có một số hạn chế xuất phát từ kiến trúc Fully Connected, chi phí dữ liệu lớn và pipeline tiền xử lý ảnh thực tế.

---

### 4.2.1. Hạn chế trong khả năng tổng quát hóa

Việc flatten ảnh $28 \times 28$ thành vector $784$ chiều làm mất cấu trúc không gian cục bộ giữa các pixel. Vì vậy, một số cặp chữ số có hình dạng gần nhau vẫn có thể bị nhầm lẫn.

---

### 4.2.2. Hạn chế khi nhận diện nét vẽ ngoài phân phối MNIST

Dữ liệu Canvas có thể khác dữ liệu huấn luyện về độ dày nét, vị trí chữ số, kích thước chữ số và nhiễu nền. Pipeline preprocessing giúp giảm khác biệt này nhưng không loại bỏ hoàn toàn rủi ro.

---

### 4.2.3. Hạn chế của quá trình tối ưu bằng Adam

Adam Optimizer giúp quá trình huấn luyện ổn định hơn trên dữ liệu lớn, nhưng vẫn cần lựa chọn learning rate, decay, weight decay và batch size phù hợp. Mô hình Deep ANN cũng có nhiều tham số, làm tăng chi phí tính toán và bộ nhớ.
...
Các hướng phát triển gồm cải thiện preprocessing, xây dựng tập kiểm thử Canvas có nhãn, thử nghiệm CNN và tăng khả năng giải thích kết quả dự đoán.
...
Phụ lục trình bày các nội dung kỹ thuật bổ trợ cho phần chính của báo cáo, bao gồm mã nguồn inference, mô tả script tạo dataset, mô tả script huấn luyện, bảng kết quả thực nghiệm, confusion matrix, mẫu dự đoán sai và pipeline tiền xử lý ảnh Canvas.

---

## Phụ lục A. Mã nguồn inference trong `ann.py`

`ann.py` hiện được module hóa cho inference, không còn chứa logic đọc dữ liệu huấn luyện hoặc huấn luyện. Nội dung chính gồm ReLU, Softmax, `forward_prop`, `load_model` và `get_predictions`.
...
## Phụ lục D. Confusion matrix và ảnh dự đoán sai bổ sung

Confusion matrix và ảnh dự đoán sai được sinh từ 10,000 mẫu đầu tiên của Infinity Dataset bằng `scripts/generate_report_assets.py`.
...
Pipeline xử lý ảnh Canvas gồm decode ảnh, grayscale, invert màu, threshold, tìm bounding box, crop, pad thành ảnh vuông, resize về $28 \times 28$, normalize và flatten thành vector $784 \times 1$.
